...
if akun['token1'] == "Ersf4igp0CULrrhu6eP1.w2CFquIQw77bHRw3YYLsCq.419r8DoSWIwdXvcgHVjHVHaXmReq/nONzjqqhxsM64A=":
    cl.login(qr=True)
else:
    cl.login(token=akun['token1'])
if akun['token2'] == "ErcJ2Ya00iGQKbxBu0ka.XK/PsHRmc+1Ck7MnAADbEG.WrjkYNl2YHmTo2dVNL03hVfZTlGtSzxOW5/sk6DaghE=":
    kk.login(qr=Trur)
else:
    kk.login(token=akun['token2'])
if akun['token3'] == "Er25vHEB1QjVZUKpZy03.OLZdf6q+uVq23UlfXjyvSW.mTA8tXb5Yk+MK3NIlPVsAUOJh0lOx+2LatfvCAL18yE=":
    ka.login(qr=Trur)
else:
    ka.login(token=akun['token3'])
